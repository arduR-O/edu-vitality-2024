...
def update_plot(frame):
    while not queue.empty():
        timestamp, focus_level = queue.get()
        time_data.append(timestamp)
        focus_values.append(focus_level)

        # Remove old data beyond 20 seconds
        while time_data and (time.time() - start_time - time_data[0] > 20):
            time_data.pop(0)
            focus_values.pop(0)
...
while True:
    ...
    if boolout:
        ...
        print(text)
        queue.put((time.time() - start_time, 1))
    elif attention:
        text = "Somewhat Focussed"
        print(text)
        queue.put((time.time() - start_time, 0.5))
    else:
        text = "Distracted"
        print(text)
        queue.put((time.time() - start_time, 0))
